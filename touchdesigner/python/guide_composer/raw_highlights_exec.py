...
def onTableChange(dat):
	...
	for row in dat.rows():
		trackid = row[0].val
		if trackid != "Trackid":
			op('lamps').ext.HighlighterExt.UpdatePosition(trackid, {'x': float(row[6].val), 'y': float(row[5].val), 'z': float(row[7].val)})
	return
...
def onCellChange(dat, cells, prev):
	...
	if reIndex: op('lamps').ext.HighlighterExt.reIndexHighlightsFromDat(dat)
	return

def onSizeChange(dat):
	debug('sizeChange')	
	op('lamps').ext.HighlighterExt.reIndexHighlightsFromDat(dat)
	return
